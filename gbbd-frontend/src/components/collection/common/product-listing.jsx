...
class ProductListing extends Component {
    ...
    if (this.props.match.params.id.includes(".jpg")) {
      try {
        ...
        if (response.data.items) {
          const server_img_url = `/uploads/${this.props.match.params.id}`;
      ...
      const _EXTERNAL_URL = `/collection/${this.props.match.params.id},${
        this.state.page
      }`;
  ...
  componentWillReceiveProps = async (nextProps) => {
    ...
    if (this.props.match.params.id !== nextProps.match.params.id) {
      if (nextProps.match.params.id.includes(".jpg")) {
        try {
        ...
        nextProps.match.params.id == "in-stock" ||
        nextProps.match.params.id == "pre-order"
      ) {
        const productsArray = await getAllFirestoreProducts();
        const aliProductsArray = await getAllFirestoreAliProductsList();
        console.log(aliProductsArray);
        nextProps.getAllProductsFirestore([
          ...productsArray,
          ...aliProductsArray,
        ]);
      } else {
        ...
        const _EXTERNAL_URL = `/collection/${nextProps.match.params.id},${
          this.state.page
        }`;
        ...
        console.log(response);
        if (response.data.items) {
          nextProps.setSearchedProductsArray(response.data.items.item);
        } else {
          nextProps.setSearchedProductsArray([]);
          setTimeout(() => {
            this.setState({ notFound: true });
          }, 30000);
        }
