...
class SearchedProduct extends Component {
  ...
  componentWillReceiveProps = async (nextProps) => {
    if (this.props.match.params.id !== nextProps.match.params.id) {
      ...
      if (response.data) {
        this.props.setSearchedProductDetail(response.data.item, "taobao");
      }
    }
  };
  ...
  getSingleProduct = async () => {
    const _EXTERNAL_URL = `/singleProduct/${this.props.match.params.id},taobao`;
    const response = await axios.get(_EXTERNAL_URL);
    console.log(response);
    if (response.data) {
      this.props.setSearchedProductDetail(response.data.item, "taobao");
  ...
  addToReduxAndFirestoreWishlist = (product) => {
    ...
    if (!currentUser) {
      history.push("/pages/login", {
        from: `/searched-product/${this.props.match.params.id}`,
      });
      return;
    }
