...
class Product1688 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      nav1: this.slider1,
  ...
  componentWillReceiveProps = async (nextProps) => {
    if (this.props.match.params.id !== nextProps.match.params.id) {
      const _EXTERNAL_URL = `/singleProduct/${nextProps.match.params.id},1688`;
      const response = await axios.get(_EXTERNAL_URL);
      console.log(response);
      if (response.data) {
        this.props.setSearchedProductDetail(response.data.item, "1688");
  ...
  getSingleProduct = async () => {
    const _EXTERNAL_URL = `/singleProduct/${this.props.match.params.id},1688`;
    const response = await axios.get(_EXTERNAL_URL);
    console.log(response);
    if (response.data) {
      this.props.setSearchedProductDetail(response.data.item, "1688");
  ...
  addToReduxAndFirestoreWishlist = (product) => {
    const { addToWishlist, currentUser, history } = this.props;
    if (!currentUser) {
      history.push("/pages/login", {
        from: `/1688/${this.props.match.params.id}`,
      });
      return;
    }
    ...
    auth.onAuthStateChanged(
      async (userAuth) =>
        await addWishlistTofirestore(userAuth, wishlistItemObj)
    );
    addToWishlist(wishlistItemObj);
  };
                  ...
                  <div className="row">
                    <div className="col-lg-6 product-thumbnail">
                      {!this.state.colorVariantImage ? (
                        ...
                        <ImageZoom
                          image={this.state.colorVariantImage}
                          className="img-fluid image_zoom_cls-0"
                        />
                      )}
                      <SmallImages
